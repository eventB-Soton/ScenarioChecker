...
public class UpdateStateLists  {
	...
	public void execute(Map<String, Variable> stateMap) {
		...
		if (timeGroup != null) associationGroup.dispose();
		...
		for (Class class_ : classList) {
			processClass(class_, stateMap); //, oracle);
		}

		for (Association ass : associationList) {
			processAssociation(ass, stateMap); //, oracle);
		}
	...
	private void processAssociation(Association association, Map<String, Variable> stateMap) { //, OracleHandler oracle) {
	...
	private void processClass(Class class_, Map<String, Variable> stateMap) { //, OracleHandler oracle) {
...
//									if (oracle != null ) oracle.addValueToSnapshot(statemachine.getName(), currentStateName, clock.getValue());
	...
	private String findInstantiation(Class class_) {
	...
	private String getAttributeMapping(Variable atVar, String instance) {
